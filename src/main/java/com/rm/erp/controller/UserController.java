...
public class UserController {
		...
		try {
			...
			default:
				try {
					 ...
					 * if(tenant!=null) { Long tenantId = tenant.getTenantId(); Integer userNumLimit
					 * = tenant.getUserNumLimit(); Integer billsNumLimit =
					 * tenant.getBillsNumLimit(); if(tenantId!=null) {
			...
			/**
			 * 在IE模式下，无法获取到user数据， 在此处明确添加上user信息
			 */
			if (user != null) {
				data.put("user", user);
			}
	...
	public String resetPwd(@RequestParam("id") Long id, HttpServletRequest request) throws Exception {
	...
	/**
	 * create by: cjl description: 查询分页用户列表 create time: 2019/3/8 15:08
	 * 
	 * @Param: pageSize  * @Param: currentPage  * @Param: search
	...
	/**
	 * create by: cjl description: 修改用户及机构和用户关系 create time: 2019/3/8 16:06
	 * 
	 * @Param: beanJson
	 * @return java.lang.Object
	 */
