...
import com.rm.erp.datasource.entities.UserBusiness;
...
import com.rm.erp.utils.BaseResponseInfo;
import com.rm.erp.utils.ErpInfo;
...
public class UserBusinessController {
	...
	public BaseResponseInfo getBasicData(@RequestParam(value = "KeyId") String keyId,
			@RequestParam(value = "Type") String type, HttpServletRequest request) throws Exception {
	...
	public String checkIsValueExist(@RequestParam(value = "type", required = false) String type,
			@RequestParam(value = "keyId", required = false) String keyId, HttpServletRequest request)
			throws Exception {
	...
	public BaseResponseInfo updateBtnStr(@RequestParam(value = "userBusinessId", required = false) Long userBusinessId,
			@RequestParam(value = "btnStr", required = false) String btnStr, HttpServletRequest request)
			throws Exception {
		BaseResponseInfo res = new BaseResponseInfo();
		try {
	...
	/**
	 * create by: qiankunpingtai website：https://qiankunpingtai.cn description:
	 * 批量删除用户角色模块关系信息 create time: 2019/3/28 15:47
	 * 
	 * @Param: ids
	 * @return java.lang.Object
	 *//*
		 * @RequestMapping(value = "/batchDeleteUserBusinessByIds") public Object
		 * batchDeleteUserBusinessByIds(@RequestParam("ids") String ids) throws
		 * Exception {
		 * 
		 * JSONObject result = ExceptionConstants.standardSuccess(); int i=
		 * userBusinessService.batchDeleteUserBusinessByIds(ids); if(i<1){
		 * logger.error("异常码[{}],异常提示[{}],参数,ids[{}]",
		 * ExceptionConstants.USER_BUSINESS_DELETE_FAILED_CODE,ExceptionConstants.
		 * USER_BUSINESS_DELETE_FAILED_MSG,ids); throw new
		 * BusinessRunTimeException(ExceptionConstants.USER_BUSINESS_DELETE_FAILED_CODE,
		 * ExceptionConstants.USER_BUSINESS_DELETE_FAILED_MSG); } return result; }
		 */
}
