...
import com.rm.erp.datasource.entities.UserBusinessExample;
import com.rm.erp.datasource.mappers.UserBusinessMapper;
...
public class UserBusinessService {
	...
	/*
	 * @Resource private UserBusinessMapperEx userBusinessMapperEx;
	 */
	@Resource
	...
	public int updateBtnStr(Long userBusinessId, String btnStr) throws Exception {
		/*
		 * logService.insertLog("关联关系", new
		 * StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_EDIT).append(userBusinessId
	 ...
	 * RequestContextHolder.getRequestAttributes()).getRequest());
	 * 
	 * User userInfo=userService.getCurrentUser(); String [] idArray=ids.split(",");
	 * int result=0; try{ result=
	 * userBusinessMapperEx.batchDeleteUserBusinessByIds(new
	 * Date(),userInfo==null?null:userInfo.getId(),idArray); }catch(Exception e){
	 * JshException.writeFail(logger, e); } return result; }
	 */
}
