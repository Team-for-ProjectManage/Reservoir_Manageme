...
//任务页面加载数据
function taskTableInfo(startTime,endTime){
	$('#taskTableAll').bootstrapTable('destroy');
	$('#taskTableAll').bootstrapTable({
		columns: [
			{
            	field:'checkName',
                title:'任务名称',
		...
		url: '/information/getTaskTableData',         //请求后台的URL（*）
		method: 'get',                      //请求方式（*）
		dataType: "json",
		toolbar: '#toolbar',                //工具按钮用哪个容器
		striped: true,                      //是否显示行间隔色
...
function taskClick1(){
	//$("#potalCheckRaskTable").bootstrapTable("refresh");//表格刷新数据
	taskTableInfo(null,"2020-04-13")
};

function taskClick2(){
	//$("#potalCheckRaskTable").bootstrapTable("refresh");//表格刷新数据
	taskTableInfo("2020-04-06",null)
};
